...
df = df.withColumn("Transaction_Amount", df["Transaction_Amount"].cast("float"))
...
avg_Transaction_Amount = df.agg(F.avg("Transaction_Amount").alias("Average_Transaction_Amount"))
...
avg_Transaction_Amount.coalesce(1).write.mode("overwrite").option("header", "true").csv("gs://siddiq_buc/output_new")
